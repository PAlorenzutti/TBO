...
/// @brief função de particionamento auxiliar para quicksort
/// @param a 
/// @param lo 
/// @param hi 
/// @return j como indíce do pivô na posição correta
int partition(Item *a, int lo, int hi){
    //pega a primeira e a segunda parte do subvetor;
    int i = lo;
    int j = hi + 1;

    //pega o pivô, sempre vai ser a parte debaixo do subvetor;
    Item v = a[lo];

    while(1){
        //enquanto o lo+1 (em diante) for menor que o pivô, ele continua incrementando i;
        while(less(a[++i], v)){
            //se bater o topo do subvetor, sai do loop;
            if(i == hi){
                break;
            }
        }

        //enquanto o hi-1 (em diante) for maior que o pivô, ele continua decrementando j;
        while(less(v, a[--j])){
            //se bater na parte debaixo do subvetor, sai do loop;
            if(j == lo){
                break;
            }
        }

        //se i e j se cruzarem, significa que já varreu as duas partes do vetor por completo
        if(i >= j){
            break;
        }

        //significa que ele achou um elemento maior que o pivô na primeira parte, e um menor que o pivô na segunda parte
        //dessa forma, eles precisam ser trocados;
        exch(a[i], a[j]);
    }

    //saiu do loop principal, então só resta colocar o pivô no lugar correto dele;
    //agora antes de j, só vai ter itens menores que j (não necessariamente em ordem)
    //depois de j, só itens maiores que j (não necessariamente em ordem)
    exch(a[lo], a[j]);

    //retorna a posição do pivô, para particionar em dois subvetores posteriormente;
    return j;
}

/// @brief função auxiliar de shuffle baseada em runtime da máquina (primordial pro quicksort)
/// @param a 
/// @param N 
void shuffle(Item *a, int N){
    struct timeval tv;
    gettimeofday(&tv, NULL); //pega o tempo atual para random;
    srand(tv.tv_usec); //inicializa a seed;

    //custo O(n)
    for(int i = N - 1; i > 0; i--){
        //seleciona aleatoriamente um elemento no vetor entre 0 e i
        int j = (unsigned int) (drand48() * (i + 1));
        exch(a[i], a[j]);
    }
}

/// @brief quick sort (versão 1) top down recursivo sem nenhuma otimização
/// @param a vetor
/// @param lo índice do primeiro elemento do vetor;
/// @param hi índice do último elemento do vetor;
void quick_sort(Item *a, int lo, int hi){
    ...
    //pega o pivô do vetor inteiro;
    int j = partition(a, lo, hi);
    //faz quick sort da primeira parte (top);
    quick_sort(a, lo, j - 1);
    //faz quick sort da segunda parte (down);
    quick_sort(a, j + 1, hi);
    //excluindo, obviamente, j, pois o pivô já está na posição correta;
}

/// @brief sort genérico para quick sort
/// @param a 
/// @param lo 
/// @param hi 
void sort(Item *a, int lo, int hi){
    shuffle(a, hi - lo + 1); //necessário para garantir performance;
    quick_sort(a, lo, hi);
}
...
int main(int argc, char *argv[]){
    ...
    sort(itens, 0, atoi(argv[1]) - 1);

    clock_t end = clock();

    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;

    print_itens(itens, atoi(argv[1]) - 1);
